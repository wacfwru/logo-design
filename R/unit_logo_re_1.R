...
#   tiff(file = "wacfwru_logo.tiff", height = 5.5, width = 5.5, units = "in", res = 300)
   png(file = "wacfwru_logo_re_1.png", height = 5.5, width = 5.5, units = "in", res = 300,
       bg = "transparent")
}

par(mai = rep(0,4), omi = rep(0.4,4), family = "uni_sans")

## draw map
map("state", region = "washington", fill = FALSE, col = colors$gray,
    mar = c(1,1,0,0), lwd = 1.5)

## add text
hh <- -2.5
mtext("WASHINGTON", side = 3, line = -3.4 + hh,
      cex = 4.8, adj = 0.5, col = colors$green)
par(family = "roboto")
mtext("C O O P E R A T I V E", side = 3, line = -6.35 + hh,
      cex = 3.74, adj = 0.54, col = colors$brown)
mtext("Fish and Wildlife Research Unit", side = 3, line = -8.5 + hh,
      cex = 2.46, adj = 0.53, col = colors$brown)
...
cc <- 0
bb <- 0.57
# add_image(rasters$salmon, x=0.44 + cc, y=0.27 + bb, size=0.09)
# add_image(rasters$bear, x=0.65 + cc, y=0.27 + bb, size=0.12)
# add_image(rasters$loon, x=0.85 + cc, y=0.27 + bb, size=0.07)
